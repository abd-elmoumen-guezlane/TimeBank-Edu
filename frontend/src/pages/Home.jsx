...
export default function Home() {
  ...
  const topModules = modules.filter((m) => m.status === 'published').slice(0, 3);
            ...
            {topModules.map((mod, i) => (
              <div key={mod.id} className={`card hover:shadow-md transition-shadow cursor-pointer`} onClick={() => navigate(`/modules/${mod.id}`)}>
                <div className="flex items-start gap-4 mb-4">
                  <div className={`w-12 h-12 bg-gradient-to-br ${moduleBg[i]} rounded-xl flex items-center justify-center flex-shrink-0 text-primary-600`}>
                    <ModuleIcon title={mod.title} size={26} />
                  </div>
