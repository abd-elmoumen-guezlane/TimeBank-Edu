    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Score moyen', val: '4.7', trend: '+0.2', color: 'text-yellow-600', star: true },
        ].map(s => (
          <div key={s.label} className="card">
            <p className="text-xs text-gray-500 mb-1">{s.label}</p>
            <p className={`text-2xl font-bold ${s.color} inline-flex items-center gap-1`}>
              {s.val}
              {s.star && <Star size={22} className="fill-yellow-400 text-yellow-400" aria-hidden />}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {[
              { name: 'Ahmed Moussa', score: 4.9, reviews: 23 },
              { name: 'Lina Farah', score: 4.8, reviews: 15 },
              { name: 'Yassine K.', score: 4.7, reviews: 31 },
            ].map((t) => (
              <div key={t.name} className="flex items-center justify-between gap-3">
                <span className="text-sm text-gray-700">{t.name}</span>
                <span className="text-sm font-semibold text-yellow-600 inline-flex items-center gap-1 flex-shrink-0 flex-wrap justify-end">
                  <StarRating rating={t.score} size={12} />
                  <span>{t.score}</span>
                  <span className="text-xs font-normal text-gray-400">({t.reviews} avis)</span>
                </span>
              </div>
            ))}
          </div>
